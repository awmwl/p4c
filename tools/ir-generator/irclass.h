...
inline std::ostream &operator<<(std::ostream &out, IrElement::access_t a) {
    switch (a) {
        case IrElement::Public:
            out << " public:";
            break;
        case IrElement::Protected:
            out << " protected:";
            break;
        case IrElement::Private:
            out << " private:";
            break;
    }
    return out << std::endl;
...
class IrClass : public IrElement {
 ...
 public:
    ...
    IrClass(NodeKind kind, cstring name, const std::initializer_list<IrElement *> &elements)
        : concreteParent(nullptr),
          elements(elements),
          containedIn(&IrNamespace::global()),
          local(containedIn, name),
          kind(kind),
          name(name) {
        IrNamespace::add_class(this);
...
inline std::ostream &operator<<(std::ostream &out, const LineDirective &l) {
    if (!LineDirective::inhibit) {
        ...
        if (l.reset) {
            ...
            out << "#line " << (pos.sourceLine + l.delta + 1) << " \"" << pos.fileName << '"'
                << std::endl;
        }
    }
    return out;
