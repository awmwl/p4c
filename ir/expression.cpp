...
void IR::Constant::handleOverflow(bool noWarning) {
    if (type == nullptr) BUG("{0}: Null type in typed constant", this);
    if (type->is<IR::Type_InfInt>()) return;
    auto tb = type->to<IR::Type_Bits>();
    if (tb == nullptr) {
        BUG("{0}: Unexpected type for constant {1}", this, type);
        return;
    }
    ...
    if (tb->isSigned) {
        ...
        if (value < min || value > max) {
            if (!noWarning)
                ::warning(ErrorType::WARN_OVERFLOW, "{0}: signed value does not fit in {1} bits",
                          this, width);
        ...
        if (value < 0) {
            if (!noWarning)
                ::warning(ErrorType::WARN_MISMATCH, "{0}: negative value with unsigned type", this);
        } else if ((value & mask) != value) {
            if (!noWarning)
                ::warning(ErrorType::WARN_MISMATCH, "{0}: value does not fit in {1} bits", this,
                          width);
        }

        value = value & mask;
        if (value < 0) BUG("Negative value after masking {0}", value);
    }
}
