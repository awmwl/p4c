...
class UBPFDeparserTranslationVisitor : public EBPF::CodeGenInspector {
 public:
    ...
    bool notSupported(const IR::Expression *expression) {
        ::error(ErrorType::ERR_UNSUPPORTED_ON_TARGET, "{0}: not supported in Deparser", expression);
        return false;
    }

    bool notSupported(const IR::StatOrDecl *statOrDecl) {
        ::error(ErrorType::ERR_UNSUPPORTED_ON_TARGET, "{0}: not supported in Deparser", statOrDecl);
        return false;
    }
