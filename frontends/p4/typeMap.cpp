...
bool TypeMap::typeIsEmpty(const IR::Type *type) const {
    if (auto bt = type->to<IR::Type_Bits>()) {
        ...
        if (tst->fields.size() == 0) return true;
...
void TypeMap::dbprint(std::ostream &out) const {
    ...
    for (auto it : leftValues) out << "\t" << dbp(it) << std::endl;
    out << "Constants" << std::endl;
    for (auto it : constants) out << "\t" << dbp(it) << std::endl;
...
void TypeMap::cloneExpressionProperties(const IR::Expression *to, const IR::Expression *from) {
    ...
    if (isCompileTimeConstant(from)) setCompileTimeConstant(to);
}
...
bool TypeMap::equivalent(const IR::Type *left, const IR::Type *right, bool strict) const {
    ...
    if (left->is<IR::Type_Base>() || left->is<IR::Type_Newtype>() || left->is<IR::Type_Var>() ||
        left->is<IR::Type_Name>())
    ...
    if (left->is<IR::Type_Error>()) return true;
    ...
    if (auto lv = left->to<IR::Type_P4List>()) {
        auto rv = right->to<IR::Type_P4List>();
    ...
    if (auto lp = left->to<IR::Type_Package>()) {
        ...
        if (lm->typeParameters->size() != rm->typeParameters->size()) return false;
        for (size_t i = 0; i < lm->typeParameters->size(); i++) {
            ...
            if (!equivalent(lp, rp, strict)) return false;
        }
        // Don't check the return type.
        if (lm->parameters->size() != rm->parameters->size()) return false;
        for (size_t i = 0; i < lm->parameters->size(); i++) {
            auto lp = lm->parameters->parameters.at(i);
            auto rp = rm->parameters->parameters.at(i);
            if (lp->direction != rp->direction) return false;
            if (!equivalent(lp->type, rp->type, strict)) return false;
        }
        return true;
    }
    if (auto a = left->to<IR::IApply>()) {
        return equivalent(a->getApplyMethodType(), right->to<IR::IApply>()->getApplyMethodType(),
                          strict);
    }
    if (auto ls = left->to<IR::Type_SpecializedCanonical>()) {
        auto rs = right->to<IR::Type_SpecializedCanonical>();
        if (!equivalent(ls->baseType, rs->baseType, strict)) return false;
        if (ls->arguments->size() != rs->arguments->size()) return false;
    ...
    if (left->is<IR::Type_Method>() || left->is<IR::Type_Action>()) {
        ...
        if (!equivalent(lm->returnType, rm->returnType, strict)) return false;
        if (lm->parameters->size() != rm->parameters->size()) return false;
        for (size_t i = 0; i < lm->parameters->size(); i++) {
            auto lp = lm->parameters->parameters.at(i);
            auto rp = rm->parameters->parameters.at(i);
...
bool TypeMap::implicitlyConvertibleTo(const IR::Type *from, const IR::Type *to) const {
    ...
    if (auto rt = to->to<IR::Type_BaseList>()) {
        if (auto sl = from->to<IR::Type_StructLike>()) {
            ...
            if (sl->fields.size() != rt->components.size()) return false;
            for (size_t i = 0; i < rt->components.size(); i++) {
                auto fl = sl->fields.at(i);
                auto r = rt->components.at(i);
                if (!implicitlyConvertibleTo(fl->type, r)) return false;
            }
            return true;
        } else if (auto sl = from->to<IR::Type_Tuple>()) {
            // We allow implicit casts from list types to tuples
            if (sl->components.size() != rt->components.size()) return false;
            for (size_t i = 0; i < rt->components.size(); i++) {
                auto f = sl->components.at(i);
                auto r = rt->components.at(i);
                if (!implicitlyConvertibleTo(f, r)) return false;
            }
            return true;
        }
    }
